import model from "../Models/productType.js";
  ...
  model
    ...
    .then((response) => {
      if (response) {
        ...
        error(res, 400, "Product Type Not Found!");
      }
    })
    .catch((err) => {
      error(res, 400, err);
...
export const post = async (req, res) => {
  try {
    ...
    return success(
      res,
      200,
      `Product Type ${response.name} created successfully!`
    );
  } catch (err) {
    console.error("Error in post productType controller:", err);
    return error(res, 400, err.message || "Post Product Type failed!");
  }
};
...
export const put = (req, res) => {
  model
    .put(req)
    .then((response) => {
      success(res, 200, `Product Type ${response.name} updated successfully`);
    })
    .catch((err) => {
      error(res, 400, err);
    });
        ...
        model
          ...
          .then((response) => {
            success(
              res,
              200,
              `Product Type ${response.name} deleted successfully`
            );
          })
          .catch((err) => {
            error(res, 400, err);
